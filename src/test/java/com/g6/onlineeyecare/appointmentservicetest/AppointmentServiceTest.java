...
public class AppointmentServiceTest {
	...
	public void testViewByIdforInvalidEntries() throws AppointmentIdNotFoundException {
		...
		when(repository.findById(1)).thenReturn(optional);
		Appointment a=appointmentService.viewAppointment(2);
		verify(repository).findById(appointment.getAppointmentId());
	...
	public void testBookAppointment() throws DoctorIdNotFoundException, PatientIdFoundNotException
	{
	     ...
	     Doctor d = new Doctor("11:30:00", 635241589, "abc@gmail.com", "bangalore");
	     d.setUserId(5);
		
	     Doctor d1 = new Doctor("11:30:00", 635241589, "abc@gmail.com", "bangalore");
	     d1.setUserId(5);
	     Optional<Doctor> expectedD=Optional.of(d1);
	     
	     when(doctorRepository.findById(d.getUserId())).thenReturn(expectedD);
	         
	     
		Appointment appointment=new Appointment(10, LocalDate.now(), LocalTime.now(),expectedD.get(),expectedP.get());
		Appointment ExcepectAppointment=new Appointment(10, LocalDate.now(), LocalTime.now(),expectedD.get(),expectedP.get());
